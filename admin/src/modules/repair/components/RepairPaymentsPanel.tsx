...
import { RepairPaymentFormDialog } from '@/modules/repair/components/RepairPaymentFormDialog';
import { useRepairPaymentBalance, useRepairPayments } from '@/modules/repair/hooks/useRepairPayments';
import type { PaymentResponse } from '@/modules/sale/types/Sale';
import { getApiErrorMessage } from '@/utils/apiError';
import { formatCurrency } from '@/utils/formatCurrency';
import { formatDateTime } from '@/utils/formatDate';

interface RepairPaymentsPanelProps {
  readonly repairId: string;
  readonly canRecordPayment: boolean;
}

/** Payments panel on the repair detail screen — uses generic payments with REPAIR reference. */
export function RepairPaymentsPanel({ repairId, canRecordPayment }: RepairPaymentsPanelProps): JSX.Element {
  const [page, setPage] = useState(DEFAULT_PAGE_INDEX);
  const [pageSize, setPageSize] = useState(DEFAULT_PAGE_SIZE);
  const [paymentOpen, setPaymentOpen] = useState(false);

  const paymentsQuery = useRepairPayments(repairId);
